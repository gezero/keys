...
//        checkArgument(hexString.length() == 64);
        this.bytes = Base16Encoder.decode(hexString);
    }
    ...
    public static Sha256Hash wrap(String hexString) {
        return wrap(Base16Encoder.decode(hexString));
    }
    ...
    public int hashCode() {
        ...
        byte[] arr = {bytes[28], bytes[29], bytes[30], bytes[31]};
        ByteBuffer wrapped = ByteBuffer.wrap(arr); // big-endian by default but was here originally big endian? hard to say...
